...
class CreatePsEndpointsTable extends Migration {
	...
	public function up()
	{
		Schema::create('ps_endpoints', function(Blueprint $table)
		{
			...
			$table->enum('ice_support', array('yes','no'))->default('yes');
			...
			$table->enum('callerid_privacy', array('allowed_not_screened','allowed_failed_screened','allowed','prohib_not_screened','prohib_passed_screened','prohib_failed_screened','prohib','unavailable'))->nullable();
			...
			$table->enum('use_avpf', array('yes','no'))->default('yes');
			$table->enum('media_encryption', array('no','sdes','dtls'))->default('dtls');
			...
			$table->string('dtls_verify', 40)->default('fingerprint');
			$table->string('dtls_rekey', 40)->nullable();
			$table->string('dtls_cert_file', 200)->default('/etc/asterisk/keys/asterisk.pem');
			$table->string('dtls_private_key', 200)->nullable();
			$table->string('dtls_cipher', 200)->nullable();
			$table->string('dtls_ca_file', 200)->default('/etc/asterisk/keys/ca.crt');
			$table->string('dtls_ca_path', 200)->nullable();
			$table->enum('dtls_setup', array('active','passive','actpass'))->default('actpass');
			...
			$table->enum('media_use_received_transport', array('yes','no'))->default('yes');
			$table->string('accountcode', 80)->nullable();
			$table->enum('user_eq_phone', array('yes','no'))->nullable();
			$table->enum('moh_passthrough', array('yes','no'))->nullable();
