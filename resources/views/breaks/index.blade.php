...
@section('page-title', 'Breaks')

@section('page_sub_title', 'All Breaks')

@section('new_record_link', action('AgentBreakController@create'))
		...
		<tbody>
			@foreach($breaks as $break)
				<tr>
					...
					<td>{{ $break->name }}</td>
					<td>
						<a href="{{ action('AgentBreakController@edit', $break->id) }}" class="btn btn-success m-btn m-btn--icon m-btn--icon-only">
							<i class="la la-edit"></i>
						</a>
						{!! Form::open(['action' => ['AgentBreakController@destroy', $break->id], 'method' => 'DELETE', 'style' => 'display:inline']) !!}
