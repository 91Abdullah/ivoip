...
@section('page-title', 'Blended Agents')

@section('page_sub_title', 'All Blended Agents')

@section('new_record_link', action('BlendedAgentController@create'))
		...
		<tbody>
			@foreach($agents as $agent)
				<tr>
					...
					<td>{{ $agent->secret }}</td>
					<td>
						<a href="{{ action('BlendedAgentController@edit', $agent->id) }}" class="btn btn-success m-btn m-btn--icon m-btn--icon-only">
							<i class="la la-edit"></i>
						</a>
						{!! Form::open(['action' => ['BlendedAgentController@destroy', $agent->id], 'method' => 'DELETE', 'style' => 'display:inline']) !!}
