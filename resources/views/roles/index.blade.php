...
@section('page_sub_title', 'All Roles')

@section('new_record_link', action('RoleController@create'))
		...
		<tbody>
			@foreach($roles as $role)
				<tr>
					...
					<td>{{ $role->name }}</td>
					<td>
						<a href="{{ action('RoleController@edit', $role->id) }}" class="btn btn-success m-btn m-btn--icon m-btn--icon-only">
							<i class="la la-edit"></i>
						</a>
						{!! Form::open(['action' => ['RoleController@destroy', $role->id], 'method' => 'DELETE', 'style' => 'display:inline']) !!}
